...
function clickEdit(numberOfClass){
  ...
  // }
  let update = "";
  if(numberOfClass === 1 || Object.values(document.querySelectorAll('li')).indexOf(liEdit) === 0){
    update = liEdit.parentElement
    // console.log(update)
    liEdit.remove()
    update.insertAdjacentHTML('afterbegin',`<li id="${numberOfClass}"><input type="text" value="${editText}" /><button class="${numberOfClass}" onclick="clickOk(${numberOfClass})">OK</button><button class="${numberOfClass}" onclick="clickDel(${numberOfClass})">Del</button></li>`)
  } else {
    update = liEdit.previousSibling
    // console.log(update)
    liEdit.remove()
    update.insertAdjacentHTML('afterend',`<li id="${numberOfClass}"><input type="text" value="${editText}" /><button class="${numberOfClass}" onclick="clickOk(${numberOfClass})">OK</button><button class="${numberOfClass}" onclick="clickDel(${numberOfClass})">Del</button></li>`)
  }
...
function clickOk(numberOfClass) {
  ...
  if(numberOfClass === 1 || Object.values(document.querySelectorAll('li')).indexOf(liOK) === 0){
    update = liOK.parentElement
    // console.log(update)
    liOK.remove()
    update.insertAdjacentHTML('afterbegin',`<li id="${numberOfClass}"><span onclick="toggle(${numberOfClass})">${text}</span><button class="${numberOfClass}" onclick="clickEdit(${numberOfClass})">Edit</button><button class="${numberOfClass}" onclick="clickDel(${numberOfClass})">Del</button></li>`)
  } else {
    update = liOK.previousSibling
    // console.log(update)
    liOK.remove()
    update.insertAdjacentHTML('afterend',`<li id="${numberOfClass}"><span onclick="toggle(${numberOfClass})">${text}</span><button class="${numberOfClass}" onclick="clickEdit(${numberOfClass})">Edit</button><button class="${numberOfClass}" onclick="clickDel(${numberOfClass})">Del</button></li>`)
  }
}
